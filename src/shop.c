...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if (((IS_GOOD(ch) && NOTRADE_GOOD(shop_nr)) ||
      ...
      !IS_AFFECTED(ch, AFF_CONCEAL_ALIGN)) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s %s", chname, MSG_NO_SELL_ALIGN);
    FREE_NAME(chname);
  ...
  if ((IS_MAGE(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
    ...
    GET_NAME(ch, chname); 
    sprintf(buf, "%s %s", chname, MSG_NO_SELL_CLASS);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return (FALSE);
  }
...
int trade_with(struct obj_data * item, int shop_nr)
{
  ...
  if (GET_OBJ_LEVEL(item) < SHOP_MIN_OBJ_LEVEL(shop_nr))
    return (OBJECT_TOO_LOW_LEVEL);

  if (GET_OBJ_LEVEL(item) > SHOP_MAX_OBJ_LEVEL(shop_nr))
    return (OBJECT_TOO_HIGH_LEVEL);
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  do {
    ...
    if (!obj) {
      if (msg) {
        GET_NAME(ch, chname);
	sprintf(buf, shop_index[shop_nr].no_such_item1, chname);
        FREE_NAME(chname);
	do_tell(keeper, buf, cmd_tell, 0);
      }
      return (0);
...
int buy_price(struct char_data* ch, struct obj_data * obj, int shop_nr)
{
  int objCost = GET_OBJ_COST(obj);
  int chCha   = 0;

  /* UGLY PEOPLE MUST PAY!  CHA FACTORS INTO BUYS HERE. */
  ...
  objCost = (int) ((float) objCost * SHOP_BUYPROFIT(shop_nr));
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  if ((buynum = transaction_amt(arg)) < 0) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s A negative amount?  Try selling me something.", chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  if (!(*arg) || !(buynum)) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s What do you want to buy??", chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  if ((buy_price(ch, obj, shop_nr) > money_to_copper(GET_MONEY(ch))) &&
      !IS_GOD(ch)) {
    GET_NAME(ch, chname);
    sprintf(buf, shop_index[shop_nr].missing_cash2, chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);

    switch (SHOP_BROKE_TEMPER(shop_nr)) {
/*  Puking on customers is just silly --Gilean
    case 0:
      GET_NAME(ch, chname);
      do_action(keeper, chname, cmd_puke, 0);
      FREE_NAME(chname);
      return;
*/
    case 0:
      GET_NAME(ch, chname);
      do_echo(keeper, "yells about charity cases trying to mooch his goods.", cmd_emote, SCMD_EMOTE);
      FREE_NAME(chname);
    return;
    case 1:
  ...
  while ((obj) &&
	 ((money_to_copper(GET_MONEY(ch)) >= buy_price(ch, obj, shop_nr)) ||
	  IS_GOD(ch))
    ...
    if (!IS_GOD(ch)) {
      ...
      sub.copper = buy_price(ch, obj, shop_nr);
  ...
  if (bought < buynum) {
    ...
    {
      GET_NAME(ch, chname);
      sprintf(buf, "%s I only have %d to sell you.", chname, bought);
      FREE_NAME(chname);
    }
    else if ( money_to_copper(GET_MONEY(ch)) < buy_price(ch, obj, shop_nr))
    {
      GET_NAME(ch, chname);
      sprintf(buf, "%s You can only afford %d.", chname, bought);
      FREE_NAME(chname);
    }
    else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
    {
      GET_NAME(ch, chname);
      sprintf(buf, "%s You can only hold %d.", chname, bought);
      FREE_NAME(chname);
    }
    else if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
    {
      GET_NAME(ch, chname);
      sprintf(buf, "%s You can only carry %d.", chname, bought);
      FREE_NAME(chname);
    }
    else
    {
      GET_NAME(ch, chname);
      sprintf(buf, "%s Something screwy only gave you %d.", chname, bought);
      FREE_NAME(chname);
    }
    do_tell(keeper, buf, cmd_tell, 0);
  }
  ...
  if (!IS_GOD(ch)) {
    (GET_MONEY(keeper)).copper += goldamt;
  }
  ...
  copper_to_money( &goldBuyAmt, goldamt);
  GET_NAME(ch, chname);
  sprintf(buf, shop_index[shop_nr].message_buy, chname,
	  describe_money(goldBuyAmt));
  FREE_NAME(chname);
  do_tell(keeper, buf, cmd_tell, 0);
  ...
  if (SHOP_USES_BANK(shop_nr)) {
    if ( money_to_copper(GET_MONEY(keeper)) > MAX_OUTSIDE_BANK) {
      SHOP_BANK(shop_nr) +=
	(money_to_copper(GET_MONEY(keeper)) - MAX_OUTSIDE_BANK);
      (GET_MONEY(keeper)).platinum = 0;
      (GET_MONEY(keeper)).steel = 0;
      (GET_MONEY(keeper)).gold = 0;
      (GET_MONEY(keeper)).copper = MAX_OUTSIDE_BANK;
    }
  }
}
...
/* Ok. This is good code, but we want more specific messages.
   So...
  if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
      (IS_OBJ_STAT(obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL(ch)) ||
      (IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) && IS_GOOD(ch)) ||
      (GET_OBJ_LEVEL(obj) > (GET_LEVEL(ch) + 7)) ||
       invalid_class(ch, obj) || invalid_race(ch, obj))
  {
    act("You fumble around with $p.", FALSE, ch, obj, 0, TO_CHAR);
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  if (!(obj = get_obj_in_list_vis(ch, name, ch->carrying))) {
    if (msg) {
      GET_NAME(ch, chname);
      sprintf(buf, shop_index[shop_nr].no_such_item2, chname);
      FREE_NAME(chname);
      do_tell(keeper, buf, cmd_tell, 0);
    }
    return (0);
  ...
  switch (result) {
  case OBJECT_TOO_LOW_LEVEL:
    GET_NAME(ch, chname);
    sprintf(buf, "%s You cannot sell that here.  It's quality is too poor!",
      chname);
    FREE_NAME(chname);
    break;
  case OBJECT_TOO_HIGH_LEVEL:
    GET_NAME(ch, chname);
    sprintf(buf, "%s You cannot sell that here.  I cannot afford to buy it!",
      chname);
    FREE_NAME(chname);
    break;
  case OBJECT_NOVAL:
  ...
  case OBJECT_NOTOK:
    GET_NAME(ch, chname);
    sprintf(buf, shop_index[shop_nr].do_not_buy, chname);
    FREE_NAME(chname);
    break;
  case OBJECT_DEAD:
    GET_NAME(ch, chname);
    sprintf(buf, "%s %s", chname, MSG_NO_USED_WANDSTAFF);
    FREE_NAME(chname);
    break;
  default:
    ...
    log(buf);
    GET_NAME(ch, chname);
    sprintf(buf, "%s An error has occurred.", chname);
    FREE_NAME(chname);
    break;
  }
  if (msg)
...
int sell_price(struct char_data * ch, struct obj_data * obj, int shop_nr)
{
  ...
  /* UGLY PEOPLE MUST PAY!  CHA FACTORS INTO SELLS HERE. */
  ...
  {
    objCost = (int)((float) objCost * 0.7);
  }
  else                 /* CHA 1 - 3 */
  {
    objCost = (int)((float) objCost * 0.6);
  }

  objCost = (int) ((float) objCost * SHOP_SELLPROFIT(shop_nr));
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  if ((sellnum = transaction_amt(arg)) < 0) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s A negative amount?  Try buying something.", chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  if (!(*arg) || !(sellnum)) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s What do you want to sell??", chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  if ( (money_to_copper(GET_MONEY(keeper)) + SHOP_BANK(shop_nr)) <
       sell_price(ch, obj, shop_nr)) {
    GET_NAME(ch, chname);
    sprintf(buf, shop_index[shop_nr].missing_cash1, chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  while ((obj) &&
	 ((money_to_copper(GET_MONEY(keeper)) + SHOP_BANK(shop_nr)) >=
	  sell_price(ch, obj, shop_nr)) &&
	 (sold < sellnum)) {
    struct money_data sub;

    sub.platinum = 0;    sub.steel = 0;    sub.gold = 0;
    sub.copper = sell_price(ch, obj, shop_nr);

    sold++;

    goldamt += sell_price(ch, obj, shop_nr);
    money_subtract( &(GET_MONEY(keeper)), sub);
  ...
  if (sold < sellnum) {
    if (!obj) {
      GET_NAME(ch, chname);
      sprintf(buf, "%s You only have %d of those.", chname, sold);
      FREE_NAME(chname);

    } else if ( (money_to_copper(GET_MONEY(keeper)) + SHOP_BANK(shop_nr)) <
		sell_price(ch, obj, shop_nr)) {
      GET_NAME(ch, chname);
      sprintf(buf, "%s I can only afford to buy %d of those.", chname, sold);
      FREE_NAME(chname);

    } else {
      GET_NAME(ch, chname);
      sprintf(buf, "%s Something really screwy made me buy %d.", chname, sold);
      FREE_NAME(chname);
    }
  ...
  {
    struct money_data moneyAdd;
    copper_to_money(&moneyAdd, goldamt);

    (GET_MONEY(ch)).platinum += moneyAdd.platinum;
    (GET_MONEY(ch)).steel += moneyAdd.steel;
    (GET_MONEY(ch)).gold += moneyAdd.gold;
    (GET_MONEY(ch)).copper += moneyAdd.copper;
  }
  ...
  copper_to_money( &goldSellAmt, goldamt);
  GET_NAME(ch, chname);
  sprintf(buf, shop_index[shop_nr].message_sell, chname, 
    describe_money(goldSellAmt));
  FREE_NAME(chname);
  do_tell(keeper, buf, cmd_tell, 0);
  sprintf(buf, "The shopkeeper now has %s.\n\r", tempstr);
  send_to_char(buf, ch);

  if ( money_to_copper(GET_MONEY(keeper)) < MIN_OUTSIDE_BANK) {
    struct money_data moneyAdd;

    goldamt = MIN(MAX_OUTSIDE_BANK - money_to_copper(GET_MONEY(keeper)),
		  SHOP_BANK(shop_nr));
    SHOP_BANK(shop_nr) -= goldamt;
    copper_to_money(&moneyAdd, goldamt);

    (GET_MONEY(keeper)).platinum += moneyAdd.platinum;
    (GET_MONEY(keeper)).steel += moneyAdd.steel;
    (GET_MONEY(keeper)).gold += moneyAdd.gold;
    (GET_MONEY(keeper)).copper += moneyAdd.copper;
  }
}
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (!(*arg)) {
    GET_NAME(ch, chname);
    sprintf(buf, "%s What do you want me to valuate??", chname);
    FREE_NAME(chname);
    do_tell(keeper, buf, cmd_tell, 0);
    return;
  }
  ...
  copper_to_money(&money, sell_price(ch, obj, shop_nr));
  GET_NAME(ch, chname);
  sprintf(buf, "%s I'll give you %s for that!", chname, describe_money(money));
  FREE_NAME(chname);
  do_tell(keeper, buf, cmd_tell, 0);
...
char *list_object(struct char_data* ch, struct obj_data * obj, int cnt, 
  int index, int shop_nr)
{
  static char buf[256];
  char buf2[300], buf3[200];
  struct money_data buyPrice;


  /*  no longer care about availability
  if (shop_producing(obj, shop_nr))
  ...
  copper_to_money( &buyPrice, buy_price(ch, obj, shop_nr));
  sprintf(buf2, "%-30s %24s\n\r", buf3, describe_money(buyPrice));
  strcat(buf, CAP(buf2));
  return (buf);
}
...
SPECIAL(shop_keeper)
{
  ...
  if (CMD_IS("steal")) {
    act("$N shouts, '$n is a bloody thief!!'", FALSE, ch, 0, keeper, TO_ROOM);
 act("$N shouts, 'You bloody thief!!'", FALSE, ch, 0, keeper, TO_CHAR);
   CHOOSE_NAME(ch, chname);
    do_action(keeper, chname, cmd_slap, 0);
    FREE_NAME(chname);
  ...
  if (CMD_IS("buy")) {
    ...
    shopping_try(argument, ch, keeper, shop_nr);
    return (TRUE);
  }
  return (FALSE);
}
  ...
  if (IS_NPC(victim) && (mob_index[GET_MOB_RNUM(victim)].func == shop_keeper))
    for (index = 0; index < top_shop; index++)
      if ((GET_MOB_RNUM(victim) == SHOP_KEEPER(index)) && !SHOP_KILL_CHARS(index)) {
        GET_NAME(ch, chname);
	do_action(victim, chname, cmd_slap, 0);
	sprintf(buf, "%s %s", chname, MSG_CANT_KILL_KEEPER);
        FREE_NAME(chname);
...
void boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
  ...
  while (!done) {
	...
	SHOP_ROOM(top_shop, count) = BUY_TYPE(list[count]);
      ...
      read_line(shop_f, "%d", &SHOP_MIN_OBJ_LEVEL(top_shop));
      read_line(shop_f, "%d", &SHOP_MAX_OBJ_LEVEL(top_shop));
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  if (SHOP_KEEPER(shop_nr) >= 0) {
    GET_NAME(&mob_proto[SHOP_KEEPER(shop_nr)], chname);
    ...
    if ((k = get_char_num(SHOP_KEEPER(shop_nr)))) {
      ...
      sprintf(buf, "Coins: [%s], Bank: [%9d] (Total: %d)\n\r",
	      ...
	      SHOP_BANK(shop_nr),
	      money_to_copper(GET_MONEY(k)) + SHOP_BANK(shop_nr));
    }
  ...
  sprintf(buf, "Obj Levels: [%d-%d]", SHOP_MIN_OBJ_LEVEL(shop_nr),
    SHOP_MAX_OBJ_LEVEL(shop_nr));
  send_to_char(buf, ch);
