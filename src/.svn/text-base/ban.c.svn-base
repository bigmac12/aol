...
ACMD(do_ban)
{
  ...
  *buf = '\0';
  ...
  GET_NAME(ch, chname);
  strncpy(ban_node->name, chname, MAX_NAME_LENGTH);
  FREE_NAME(chname);
  ...
  GET_NAME(ch, chname);
  sprintf(buf, "%s has banned %s for %s players.", chname, site,
...
ACMD(do_unban)
{
  ...
  sprintf(buf, "%s removed the %s-player ban on %s.",
	  chname, ban_types[ban_node->type], ban_node->site);
  FREE_NAME(chname);
  mudlog(buf, NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE);
...
int Valid_Name(char *newname)
{
  ...
  for (dt = descriptor_list; dt; dt = dt->next)
  {
    ...
    {
      GET_NAME(dt->character, chname);
  
      if (!str_cmp(chname, newname))
        returnValue = (STATE(dt) == CON_PLAYING);
