...
void another_hour(int mode)
{
  ...
  if (mode) {
    ...
    case 21:
      ...
      send_to_outdoor("The sun has vanished below the horizon.\r\n");
      break;
    case 22:
...
void check_digging() {
  ...
  for (d = descriptor_list; d; d = d->next) {
    ...
    if (PLR_FLAGGED(d->character, PLR_DIGGING) &&
      ROOM_FLAGGED(d->character->in_room, ROOM_NODIG))
      REMOVE_BIT(PLR_FLAGS(d->character), PLR_DIGGING);

    if (PLR_FLAGGED(d->character, PLR_DIGGING)) {

      dig = number(1, 10);
      GET_MOVE(d->character) -= 6;

    if (GET_MOVE(d->character) < 1) {
        send_to_char("You are too tired..you stop digging.\r\n", d->character);
        ...
        send_to_char("Time goes by... you shovel on.\r\n", d->character);
      } else if (dig >= 6) {
       send_to_char("Your shovel seems to have hit something.\r\n", d->character);
       SET_BIT(PLR_FLAGS(d->character), PLR_DIG_ON);
      } else if (dig >= 4) {
       send_to_char("Your shovel hits something hard.\r\n",
                     d->character);
       SET_BIT(PLR_FLAGS(d->character), PLR_DIG_ON);
      }
     else if (dig >= 2) { 
       send_to_char("Thunk! Your shovel hits something solid in the ground!\r\n", d->character);
       SET_BIT(PLR_FLAGS(d->character), PLR_DIG_ON);
      }
     else {
        send_to_char("Time goes by... you shovel on.\r\n", d->character);
