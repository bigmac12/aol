...
ACMD(do_oasis)
{
   ...
   */
  if ( ( (zone_table[OLC_ZNUM(d)].number != GET_OLC_ZONE(ch) ) || (GET_OLC_ZONE(ch) < 1) ) && (GET_LEVEL(ch) < LVL_GRGOD) ) {
...
//    mudlog(BRF, LVL_IMPL, TRUE, "OLC: %s tried to edit zone %d allowed zone %d", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch));
  ...
  mudlog(CMP, LVL_IMMORT, TRUE, "OLC: %s starts editing zone %d allowed zone %d", GET_NAME(ch), zone_table[OLC_ZNUM(d)].number, GET_OLC_ZONE(ch));
}
