...
void send_editor_help(struct descriptor_data *d)
{
  if (using_improved_editor)
    write_to_output2(d, "Instructions: /s to save, /h for more options.\r\n");
  else
    write_to_output2(d, "Instructions: Type @ on a line by itself to end.\r\n");
}
  ...
  *str = '\0';
  ...
  case 'c':
    if (*(d->str)) {
      free(*d->str);
      *(d->str) = NULL;
      write_to_output2(d, "Current buffer cleared.\r\n");
    } else
      write_to_output2(d, "Current buffer empty.\r\n");
    break;
  case 'd':
  ...
  default:
    write_to_output2(d, "Invalid option.\r\n");
    break;
  }
...
void parse_action(int command, char *string, struct descriptor_data *d)
{
  ...
  case PARSE_FORMAT:
    ...
    write_to_output2(d, "Text formatted with%s indent.\r\n", (indent ? "" : "out"));
    break;
  case PARSE_REPLACE:
    while (isalpha(string[j]) && j < 2)
    ...
    if ((s = strtok(string, "'")) == NULL) {
      write_to_output2(d, "Invalid format.\r\n");
      return;
    } else if ((s = strtok(NULL, "'")) == NULL) {
      write_to_output2(d, "Target string must be enclosed in single quotes.\r\n");
      return;
    } else if ((t = strtok(NULL, "'")) == NULL) {
      write_to_output2(d, "No replacement string.\r\n");
      return;
    } else if ((t = strtok(NULL, "'")) == NULL) {
      write_to_output2(d, "Replacement string must be enclosed in single quotes.\r\n");
      return;
    } else if ((total_len = ((strlen(t) - strlen(s)) + strlen(*d->str))) <= d->max_str) {
      if ((replaced = replace_str(d->str, s, t, rep_all, d->max_str)) > 0) {
	write_to_output2(d, "Replaced %d occurance%sof '%s' with '%s'.\r\n", replaced, ((replaced != 1) ? "s " : " "), s, t);
      } else if (replaced == 0) {
	write_to_output2(d, "String '%s' not found.\r\n", s);
      } else
	write_to_output2(d, "ERROR: Replacement string causes buffer overflow, aborted replace.\r\n");
    } else
      write_to_output2(d, "Not enough space left in buffer.\r\n");
    break;
  case PARSE_DELETE:
    switch (sscanf(string, " %d - %d ", &line_low, &line_high)) {
    case 0:
      write_to_output2(d, "You must specify a line number or range to delete.\r\n");
      return;
    case 1:
      line_high = line_low;
      break;
	...
	write_to_output2(d, "That range is invalid.\r\n");
	return;
      }
      break;
    }
    ...
    total_len = 1;
    if ((s = *d->str) == NULL) {
      write_to_output2(d, "Buffer is empty.\r\n");
      return;
    } else if (line_low > 0) {
      while (s && i < line_low)
	if ((s = strchr(s, '\n')) != NULL) {
	...
	write_to_output2(d, "Line(s) out of range; not deleting.\r\n");
	return;
      }
      t = s;
      while (s && i < line_high)
	...
	total_len--;
      *t = '\0';
      ...
      write_to_output2(d, "%d line%sdeleted.\r\n", total_len, (total_len != 1 ? "s " : " "));
    } else {
      write_to_output2(d, "Invalid, line numbers to delete must be higher than 0.\r\n");
      return;
    }
    break;
  case PARSE_LIST_NORM:
    ...
    *buf = '\0';
    ...
    if (line_low < 1) {
      write_to_output2(d, "Line numbers must be greater than 0.\r\n");
      return;
    ...
    *buf = '\0';
    ...
    if (i < line_low || s == NULL) {
      write_to_output2(d, "Line(s) out of range; no buffer listing.\r\n");
      return;
    }
    t = s;
    while (s && i <= line_high)
  ...
  case PARSE_INSERT:
    ...
    if (*buf == '\0') {
      write_to_output2(d, "You must specify a line number before which to insert text.\r\n");
      return;
    }
    line_low = atoi(buf);
    strcat(buf2, "\r\n");
    ...
    *buf = '\0';
    if ((s = *d->str) == NULL) {
      write_to_output2(d, "Buffer is empty, nowhere to insert.\r\n");
      return;
    }
    if (line_low > 0) {
      while (s && (i < line_low))
	if ((s = strchr(s, '\n')) != NULL) {
      ...
      if (i < line_low || s == NULL) {
	write_to_output2(d, "Line number out of range; insert aborted.\r\n");
	return;
      }
      temp = *s;
      *s = '\0';
	...
	*s = temp;
	write_to_output2(d, "Insert text pushes buffer over maximum size, insert aborted.\r\n");
	return;
      }
      if (*d->str && **d->str)
	strcat(buf, *d->str);
	...
	strcat(buf, s);
      ...
      strcpy(*d->str, buf);
      write_to_output2(d, "Line inserted.\r\n");
    } else {
      write_to_output2(d, "Line number must be higher than 0.\r\n");
      return;
    }
    break;
  ...
  case PARSE_EDIT:
    half_chop(string, buf, buf2);
    if (*buf == '\0') {
      write_to_output2(d, "You must specify a line number at which to change text.\r\n");
      return;
    }
    line_low = atoi(buf);
    strcat(buf2, "\r\n");
    ...
    *buf = '\0';
    if ((s = *d->str) == NULL) {
      write_to_output2(d, "Buffer is empty, nothing to change.\r\n");
      return;
    }
    if (line_low > 0) {
      /*
	...
	if ((s = strchr(s, '\n')) != NULL) {
       ...
       */
      if (s == NULL || i < line_low) {
	write_to_output2(d, "Line number out of range; change aborted.\r\n");
	return;
      }
      /*
	...
	*s = temp;
	...
	write_to_output2(d, "Change causes new length to exceed buffer maximum size, aborted.\r\n");
	return;
      }
      /*
      ...
      RECREATE(*d->str, char, strlen(buf) + 3);
      strcpy(*d->str, buf);
      write_to_output2(d, "Line changed.\r\n");
    } else {
      write_to_output2(d, "Line number must be higher than 0.\r\n");
      return;
    }
