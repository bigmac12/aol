...
void zedit_setup(struct descriptor_data *d, int room_num)
{
  ...
  strcpy(zone->owner, zone_table[OLC_ZNUM(d)].owner);
  for(i=0; i<15; i++) {
    zone->zone_levels[i] = zone_table[OLC_ZNUM(d)].zone_levels[i];
  }
...
void zedit_new_zone(struct char_data *ch, int vzone_num)
{
  ...
  /*  PDH  2/22/99  */
  GET_NAME(ch, chname);
  strcpy(new_table[i].owner, chname); 
  FREE_NAME(chname);
  new_table[i].zone_status = ZONE_WORK;
  new_table[i].zone_levels[0] = TRUE;
  for (j = 1; j < 15; j++)
    new_table[i].zone_levels[j] = FALSE;
   ...
   */
  GET_NAME(ch, chname);
  sprintf(buf, "OLC: %s creates new zone #%d", chname, vzone_num);
  FREE_NAME(chname);
...
void zedit_save_internally(struct descriptor_data *d)
{
  ...
  if (OLC_ZONE(d)->number) {
    ...
    /*  PDH  2/22/99
     *  zone state, owner and levels
     */
    zone_table[OLC_ZNUM(d)].zone_status = OLC_ZONE(d)->zone_status;
    for(i=0; i<15; i++) {
      zone_table[OLC_ZNUM(d)].zone_levels[i] = OLC_ZONE(d)->zone_levels[i];
    }
    strcpy(zone_table[OLC_ZNUM(d)].owner, OLC_ZONE(d)->owner);
  }
...
void zedit_save_to_disk(int zone_num)
{
  ...
  /*  PDH  2/22/99
   *  write out the zone state, owner and levels
   */
  fprintf(zfile, "* ZoneState %d %s %s\n", 
	  zone_table[zone_num].zone_status, zone_table[zone_num].owner,
	  areas_level(zone_table[zone_num].zone_levels, 1));
...
void zedit_disp_menu(struct descriptor_data *d)
{
   ...
   * Menu header  
   */
  switch( OLC_ZONE(d)->zone_status ) {
    case ZONE_WORK:
      strcpy(zStatus, "work");
      break;
    case ZONE_REVIEW:
      strcpy(zStatus, "review");
      break;
    case ZONE_CLOSED:
      strcpy(zStatus, "closed");
      break;
    case ZONE_OPEN:
      strcpy(zStatus, "open");
      break;
    case ZONE_EMPTY:
      strcpy(zStatus, "empty");
      break;
  default:
      strcpy(zStatus, "UNKNOWN");
      break;
  }
  ...
  sprintf(buf,
	...
	grn, nrm, yel, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : "<NONE!>",
	  ...
	  grn, nrm, yel, zStatus,
	  grn, nrm, yel, areas_level(OLC_ZONE(d)->zone_levels, 0),
...
/*-------------------------------------------------------------------*/
  case ZEDIT_MAIN_MENU:
    ...
    case 'O':
       ...
       * Edit zone owner
       */
      send_to_char("Enter zone owner (builder) : ", d->character);
    ...
    case 'S':
      ...
      send_to_char("\r\n"
		   ...
		   "4) Open\r\n"
		   "5) Empty\r\n"
		   "Enter new zone status : ", d->character);
    ...
    case 'V':
       ...
       * Edit zone levels
       */
      send_to_char("\r\nEnter zone levels, which are a bit field for each two levels.\r\n", d->character);
      send_to_char("For example, a zone of levels 5-12 would be: 001111000000000\r\n", d->character);
      send_to_char("Enter the 15 bit zone levels : ", d->character);
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_STATUS:
    /*
     * Add new status and return to main menu.
		   "1) Working\r\n"
		   "2) In Review\r\n"
		   "3) Closed\r\n"
		   "4) Open\r\n"
     ...
     */
    if ( GET_LEVEL(d->character) < LVL_GRGOD ) {
      send_to_char("You must be implementor to change this.\r\n",d->character);
      break;
    }

    pos = atoi(arg);
    if ( ! isdigit(*arg) || (pos < 1) || (pos > 5)) {
      send_to_char("Try again (1-5) : ", d->character);
    } else {
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_LEVELS:
    /*
     * Add new levels and return to main menu.
     */
    if ( GET_LEVEL(d->character) < LVL_GRGOD ) {
      send_to_char("You must be implementor to change this.\r\n",d->character);
      break;
    ...
    if ( strlen(arg) < 2 ) {
      send_to_char("Specify more levels (at least five 0 or 1's). \r\n",
		   d->character);
      break;
      ...
      for(lev=0; lev<15; lev++) {
	OLC_ZONE(d)->zone_levels[lev] = FALSE;
      }

      for(lev=0; lev<strlen(arg) && lev<15; lev++) {
	if ( arg[lev] == '1' ) {
	  OLC_ZONE(d)->zone_levels[lev] = TRUE;
	}
      }
...
/*-------------------------------------------------------------------*/
  case ZEDIT_ZONE_OWNER:
    /*
     * Add new owner and return to main menu.
     */
    if ( GET_LEVEL(d->character) < LVL_GRGOD ) {
      send_to_char("You must be implementor to change this.\r\n",d->character);
      break;
...
/*  PDH  2/25/99
 *  do_zstat - added after "show zone" changes to make available the
 *  age, top and lifecycle stats again
 */
ACMD(do_zstat)
{
  int           zone, j, i;
  ...
  sprintf(tmpBuf, "%3d %-25.25s Age: %3d; Reset: %3d (%1d); Top: %5d\r\n",
  ...
  sprintf(tmpBuf, "%3d %-25.25s : %-10s  ",
	  zone_table[zone].number, zone_table[zone].name,
	  zone_table[zone].owner);

  switch(zone_table[zone].zone_status) {
  case ZONE_WORK:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "working");
    break;
  case ZONE_REVIEW:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "in review");
    break;
  case ZONE_CLOSED:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "closed");
    break;
  case ZONE_OPEN:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "open");
    break;
  case ZONE_EMPTY:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "empty");
    break;
  default:
    sprintf(tmpBuf, "%s%-10s ", tmpBuf, "UNKNOWN");
    break;
  }

  sprintf(tmpBuf, "%sLevels [%s]\r\n", tmpBuf,
