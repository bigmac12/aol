...
ACMD(do_track)
{
  ...
  /* No way you're finding this guy --gan, 04/28/03 (Nuitari) */
  if(GET_SKILL(vict, SKILL_PASS_WO_TRACE))
  {
    percent = number(75, 150);
    if(use_skill(vict, percent, SKILL_PASS_WO_TRACE))
    {
      send_to_char("You sense no trail.\r\n", ch);
      HUNTING(ch) = NULL;
      return;
  ...
  if (number(0, 101) >= GET_SKILL(ch, SKILL_TRACK)) {
    ...
    if (CAN_GO(ch, dir))
      ...
      sprintf(buf, "You can not sense a trail from here!\r\n");
  ...
  dummy = check_skill(ch, 101, SKILL_TRACK); // for learning only
}
...
void hunt_victim(struct char_data * ch)
{
  ...
  if (!found) {
    act("$n says, 'Damn!  I've lost the trail!'", FALSE, ch, 0, 0, TO_ROOM);
    HUNTING(ch) = NULL;
    return;
  } 
 
  if (!CAN_SEE(ch, HUNTING(ch))) {
    return;
  }

  if (PRF_FLAGGED(HUNTING(ch), PRF_HUNTED) && ch->in_room == HUNTING(ch)->in_room) {
    act("$n says, 'There you are!  I knew I would find you!'", FALSE, 
      ch, 0, 0, TO_ROOM);
    hit(ch, HUNTING(ch), TYPE_UNDEFINED);
  } else if ((dir = find_first_step(ch->in_room, HUNTING(ch)->in_room)) < 0) {
    sprintf(buf, "$n says, 'Damn!  I lost %s!'", HMHR(HUNTING(ch)));
    act(buf, FALSE, ch, 0, 0, TO_ROOM);
  } else {
    perform_move(ch, dir, 1);
    if (PRF_FLAGGED(ch, PRF_HUNTED) && ch->in_room == HUNTING(ch)->in_room) {
      act("$n says, 'There you are!  I knew I would find you!'", FALSE, 
        ch, 0, 0, TO_ROOM);
      hit(ch, HUNTING(ch), TYPE_UNDEFINED);
    }
...
void continue_track(struct char_data * ch)
{
  ...
  if (!found || (!CAN_SEE(ch, HUNTING(ch))))
  {
  ...
  switch (dir)
  {
    ...
    case BFS_NO_PATH:
      ...
      return;
    case BFS_ALREADY_THERE:
      send_to_char("Your quarry stands before you!\r\n", ch);
      HUNTING(ch) = NULL;
      return;
   default:
      sprintf(buf, "You sense a trail %s from here!\r\n", dirs[dir]);
