...
void oedit_disp_val1_menu(struct descriptor_data *d)
{
  ...
  case ITEM_NOTE:
    ...
    break;  
  case ITEM_PORTAL:
    send_to_char("Place -1 here : ", d->character);
    break;
  default:
    oedit_disp_menu(d);
  }
...
void oedit_disp_val2_menu(struct descriptor_data *d)
{
  ...
  case ITEM_WEAPON:
    ...
    break;
  case ITEM_MONEY:
    send_to_char("Number of steel coins : ", d->character);
    break;
  case ITEM_RAW:
  case ITEM_FOOD:
    /*
  ...
  case ITEM_FOUNTAIN:
    ...
    break;
  case ITEM_PORTAL:
    send_to_char("Enter virtual room num you wish to go to here: ", d->character);
    break;  
  default:
    oedit_disp_menu(d);
  }
...
void oedit_disp_weapon_spells(struct descriptor_data *d)
{
  ...
  for(counter=0;counter<MAX_SPELL_AFFECT;counter++) {
    sprintf(buf,"[%s%d%s] Spell: %s%20s%s Level: %s%3d%s Percent: %s%3d%s\r\n",
            cyn, counter+1, nrm,
            cyn, spells[OLC_OBJ(d)->wpn_spells[counter].spellnum], nrm,
            cyn, OLC_OBJ(d)->wpn_spells[counter].level, nrm,
            cyn, OLC_OBJ(d)->wpn_spells[counter].percent, nrm);
    send_to_char(buf,d->character);
  }
  send_to_char("Enter spell to edit : ",d->character);
}
...
void oedit_parse(struct descriptor_data *d, char *arg)
{
  ...
  case OEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      sprintf(buf, "OLC: %s edits obj %d", chname, OLC_NUM(d));
      FREE_NAME(chname);
  ...
  case OEDIT_MAIN_MENU:
   ...
   case 'l':
    case 'L':
      send_to_char("Enter Min Level for object : ", d->character);
...
case OEDIT_LEVEL:
  ...
  case OEDIT_WEAPON_SPELLS:
    ...
    send_to_char("At what level should it be cast: ",d->character);
    return;
  ...
  case OEDIT_WEAPON_SPELL_LEVEL:
    ...
    if(number<1) {
      ...
      send_to_char("What level should the spell be cast at: ",d->character);
      return;
    }
    OLC_OBJ(d)->wpn_spells[OLC_VAL(d)].level=number;
    send_to_char("What percent of rounds should it go off: ",d->character);
  ...
  case OEDIT_WEAPON_SPELL_PERCENT:
    ...
    if(number<0 || number>100) {
     send_to_char("Invalid percent, must be 0-100.\r\nPlease enter the percent:  ",d->character);
      return;
    }
