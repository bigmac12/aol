...
void ten_second_update(void);	/* In limits.c */
...
int main(int argc, char **argv)
{
  ...
  while ((pos < argc) && (*(argv[pos]) == '-')) {
    ...
    case 'l':
      ...
      break;
    case 'C': /* -C<socket number> - recover from copy over, this is the control socket */
       fCopyOver = TRUE;
    ...
    case 'o':
      xap_objs = 1;
      puts("Loading player objects from secondary (ascii) files.");		
      break;	
    case 'h':
      ...
      /* Do NOT use -C, this is the copyover mode and without
                  ...
                  * -spl */			
      printf("Usage: %s [-c] [-o] [-m] [-q] [-r] [-s] [-d pathname] [port #]\n"
...
if ( (now.tv_sec > last_time.tv_sec) ||
     ( (now.tv_sec == last_time.tv_sec) && (now.tv_usec > last_time.tv_usec) ) ) {
PDHtimecheck = 1;
}

    } while ( (timeout.tv_usec || timeout.tv_sec) && (PDHtimecheck < 1) );
#else
    /* Go to sleep */
    do {
    ...
    for (d = descriptor_list; d; d = next_d) {
     ...
     if (d->character) {
      ...
      if (d->str)               /* Writing boards, mail, etc. */
	    ...
	    //SEND_TO_Q("NANNY SEQUENCE ENTERED\r\n", d);
        nanny(d, comm);
        }
...
void short_diagnose(struct char_data * i, struct char_data *fr,
  char retBuf[MAX_INPUT_LENGTH])
{
  int  percent;

  memset(retBuf, (char) NULL, MAX_INPUT_LENGTH);
  ...
  if ( ! CAN_SEE(fr, i) ) {
    strncpy(retBuf, "someone: ???", MAX_INPUT_LENGTH - 1);
  } else {
   CHOOSE_NAME_II(i, fr, chname);
    strncpy(retBuf, chname, MAX_INPUT_LENGTH - 1);
    FREE_NAME(chname);
    CAP(retBuf);

    if (percent >= 100)
      strcat(retBuf, ": superb");
    else if (percent >= 90)
      strcat(retBuf, ": few scratches");
    else if (percent >= 80)
      strcat(retBuf, ": nicks and cuts");
    else if (percent >= 70)
      strcat(retBuf, ": bleeding slightly");
    else if (percent >= 60)
      strcat(retBuf, ": light wounds");
    else if (percent >= 50)
      strcat(retBuf, ": moderate wounds");
    else if (percent >= 40)
      strcat(retBuf, ": wounded");
    else if (percent >= 30)
      strcat(retBuf, ": heavy wounds");
    else if (percent >= 20)
      strcat(retBuf, ": severe wounds");
    else if (percent >= 10)
      strcat(retBuf, ": fading away");
    else if (percent >= 0)
      strcat(retBuf, ": near death");
    else
      strcat(retBuf, ": brink of death ");
  }
}
...
char *make_prompt(struct descriptor_data *d)
{
  ...
  memset(prompt, (char) NULL, MAX_PROMPT_LENGTH + 1);
  ...
  if (d->str)
    ...
    int count = 0;
    struct char_data *attackee, *opponent;
    char fightProm[MAX_INPUT_LENGTH];
    char retBuf[MAX_INPUT_LENGTH];
    ...
    *fightProm = '\0';
    ...
    if ( d != NULL ) {
      if ( d->original != NULL ) {
	if ( d->original->char_specials.position == POS_FIGHTING ) {

	  attackee = d->original->char_specials.fighting;

	  if ( attackee != NULL ) {
	    opponent = FIGHTING(attackee);

	    if ( opponent != NULL ) {
	      short_diagnose(attackee, d->original, retBuf);
	      count += sprintf(fightProm, "%s  ", retBuf);
	      short_diagnose(opponent, d->original, retBuf);
	      count += sprintf(fightProm, "%s%s ", fightProm, retBuf);
	    }
	  }
	}
      } else if ( d->character != NULL ) {
	if ( d->character->char_specials.position == POS_FIGHTING ) {
	  attackee = d->character->char_specials.fighting;

	  if ( attackee != NULL ) {
	    opponent = FIGHTING(attackee);

	    if ( opponent != NULL ) {
	      short_diagnose(attackee, d->character, retBuf);
	      count += sprintf(fightProm, "%s  ", retBuf);
	      short_diagnose(opponent, d->character, retBuf);
	      count += sprintf(fightProm, "%s%s ", fightProm, retBuf);
	    }
	  }
    ...
    *prompt = '\0';
    ...
    if (GET_INVIS_LEV(d->character))
      count += sprintf(prompt, "i%d ", GET_INVIS_LEV(d->character));

    if (PRF_FLAGGED(d->character, PRF_DISPHP))
      count += sprintf(prompt, "%s%dH ", prompt, GET_HIT(d->character));

    if (PRF_FLAGGED(d->character, PRF_DISPMOVE))
      count += sprintf(prompt, "%s%dV ", prompt, GET_MOVE(d->character));
...
// New exp % code by Gicker
  ...
  CHOOSE_NAME(d->character, chname);
    sprintf(prompt, "%s> ", chname);
    FREE_NAME(chname);
 } else
    *prompt = '\0';
...
int process_output(struct descriptor_data *t)
{
  ...
  memset(i, (char) NULL, MAX_SOCK_BUF);
...
int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
  ...
  *(second++) = '\0';
  ...
  /* now, we construct the newsub string for output. */
    ...
    if (d->character->player.name)
    {
      ...
      if (STATE(d) == CON_PLAYING || STATE(d) == CON_DISCONNECT) {
        ...
        if (!IS_NPC(d->character)) {
          int saveRoom = NOWHERE;
          saveRoom = real_room(GET_LOADROOM(d->character));
          save_char(d->character, saveRoom);
          sprintf(buf, "Closing link to: %s.", chname);
          mudlog(buf, NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE);
        }
        d->character->desc = NULL;
      } else {
        die_follower(d->character);
        sprintf(buf, "Losing player: %s.", (chname == NULL) ? chname : "<null>");
        mudlog(buf, NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE);
...
RETSIGTYPE intsig(int sig)
{
  log("SYSERR: Received SIGINT, shutting down...");
  exit(0);			/* perhaps something more elegant should
				 * substituted */
}

RETSIGTYPE termsig(int sig)
{
...
RETSIGTYPE hupsig(int sig)
{
  log("SYSERR: Received SIGHUP, shutting down...");
  exit(0);			/* perhaps something more elegant should
				 * substituted */
}
...
static char *ACTNULL = "<NULL>";

#if 1
#define CHECK_NULL(pointer, expression) \
  if ((pointer) == NULL) strcpy(tbuf, ACTNULL); else strcpy(tbuf, expression);
#else
#define CHECK_NULL(pointer, expression) \
		 ...
		 const void *vict_obj, const struct char_data *to)
{
  ...
  for (;;) {
    memset(tbuf, (char) NULL, MAX_STRING_LENGTH);
    if (*orig == '$') {
      switch (*(++orig)) {
      case 'n':
        GET_PERS(ch, to, chname);
        strcpy(tbuf, chname);
        FREE_NAME(chname);
	/* i = PERS(ch, to); */
	break;
      case 'N':
         GET_PERS((struct char_data*) vict_obj, to, chname);

	CHECK_NULL(vict_obj, chname);
        FREE_NAME(chname);
        dg_victim = (struct char_data *) vict_obj;
	break;
      case 'k':
        GET_PERS_II(ch, to, chname);
        strcpy(tbuf, chname);
        FREE_NAME(chname);
        /* i = PERS(ch, to); */
        break;
      case 'K':
         GET_PERS_II((struct char_data*) vict_obj, to, chname);
	...
	break;
      case 'M':
        if (PRF_FLAGGED((struct char_data *) vict_obj, PRF_NOTSELF)) {
        CHECK_NULL(vict_obj, HMHRD((struct char_data *) vict_obj));
        dg_victim = (struct char_data *) vict_obj;
        }
       else {
	...
	break;
      case 'S':
          if (PRF_FLAGGED((struct char_data *) vict_obj, PRF_NOTSELF)) {
          CHECK_NULL(vict_obj, HSHRD((struct char_data *) vict_obj));
        dg_victim = (struct char_data *) vict_obj;
          }
         else {
	...
	break;
      case 'E':
        if (PRF_FLAGGED((struct char_data *) vict_obj, PRF_NOTSELF)) {
        CHECK_NULL(vict_obj, HSSHD((struct char_data *) vict_obj));
        dg_victim = (struct char_data *) vict_obj;
         }
        else {
	...
	break;
      case 'p':
         if (GET_OBJ_CREATION(obj) == 1 && GET_OBJ_NATURE(obj) == 1 && GET_CLASS(to) == CLASS_MAGE) {
            CHECK_NULL(obj, OBJT(obj, to));
          }
         else if (GET_OBJ_CREATION(obj) == 1 && GET_OBJ_NATURE(obj) == 2 && GET_CLASS(to) == CLASS_CLERIC) {
            CHECK_NULL(obj, OBJT(obj, to));
          }
 ...
 if (GET_OBJ_CREATION(obj) ==1 && GET_OBJ_NATURE(obj) == 1 && GET_CLASS((struct char_data *) vict_obj) == CLASS_MAGE) {
        CHECK_NULL(vict_obj, OBJT((struct obj_data *) vict_obj, to));
        dg_target = (struct obj_data *) vict_obj;
         }

else if (GET_OBJ_CREATION(obj) ==1 && GET_OBJ_NATURE(obj) == 2 && GET_CLASS((struct char_data *) vict_obj) == CLASS_CLERIC) {
        CHECK_NULL(vict_obj, OBJT((struct obj_data *) vict_obj, to));
        dg_target = (struct obj_data *) vict_obj;
         }
        else {
	 ...
	 struct obj_data *obj, const void *vict_obj, int type)
{
   ...
   *
   * So is TO_NOTDEAF.  Soli, 9/15/99
   */

  /* check if TO_SLEEP is there, and remove it if it is. */
  if ((sleep = (type & TO_SLEEP)))
    type &= ~TO_SLEEP;
  if ((deaf = (type & TO_NOTDEAF)))
    type &= ~TO_NOTDEAF;
