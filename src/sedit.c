...
#define S_MIN_OBJ_LEVEL(i)      ((i)->minObjLevel)
#define S_MAX_OBJ_LEVEL(i)      ((i)->maxObjLevel)
...
void sedit_setup_new(struct descriptor_data *d)
{
  ...
  S_MIN_OBJ_LEVEL(shop) = 1;
  S_MAX_OBJ_LEVEL(shop) = 38;
  ...
  S_BUY(shop) = str_dup("%s That'll be %s, thanks.");
  S_SELL(shop) = str_dup("%s I'll give you %s for that.");
  /*
...
void copy_shop(struct shop_data *tshop, struct shop_data *fshop)
{
  ...
  S_MIN_OBJ_LEVEL(tshop) = S_MIN_OBJ_LEVEL(fshop);
  S_MAX_OBJ_LEVEL(tshop) = S_MAX_OBJ_LEVEL(fshop);
...
void sedit_add_to_int_list(sh_int **list, sh_int new)
{
...
void sedit_remove_from_int_list(sh_int **list, sh_int num)
{
  sh_int i, num_items, *nlist;
...
void sedit_save_to_disk(int zone_num)
{
  ...
  for (i = zone * 100; i <= top; i++) {
	...
	fprintf(shop_file, "%d\n", S_ROOM(shop, j));
      ...
      fprintf(shop_file, "%d\n%d\n", S_MIN_OBJ_LEVEL(shop), 
		S_MAX_OBJ_LEVEL(shop));
    }
  }
  fprintf(shop_file, "$~\n");
  fclose(shop_file);
...
void sedit_disp_menu(struct descriptor_data *d)
{
  ...
  sprintf(buf,
   ...
   "%s3%s) Open 2      : %s%4d%s          %s4%s) Close 2     : %s%4d\r\n"
	  ...
	  "%sF%s) Shop flags     : %s%s\r\n"
          "%sG%s) Min Obj Lvl    : %s%4d\r\n"
          "%sH%s) Max Obj Lvl    : %s%4d\r\n"
	  "%sR%s) Rooms Menu\r\n"
	  "%sP%s) Products Menu\r\n"
	  ...
	  grn, nrm, cyn, buf1,
	  grn, nrm, cyn, buf2,
	  grn, nrm, cyn, S_MIN_OBJ_LEVEL(shop),
	  grn, nrm, cyn, S_MAX_OBJ_LEVEL(shop),
...
/*-------------------------------------------------------------------*/
  case SEDIT_CONFIRM_SAVESTRING:
    ...
    case 'Y':
      ...
      sprintf(buf, "OLC: %s edits shop %d", chname, OLC_NUM(d));
      FREE_NAME(chname);
...
/*-------------------------------------------------------------------*/
  case SEDIT_MAIN_MENU:
    ...
    case 'g':
    case 'G':
      OLC_MODE(d) = SEDIT_MIN_OBJ_LEVEL;
      i++;
      break;
    case 'h':
    case 'H':
      OLC_MODE(d) = SEDIT_MAX_OBJ_LEVEL;
      i++;
      break;
    case 'r':
    case 'R':
...
/*-------------------------------------------------------------------*/
  ...
  case SEDIT_CLOSE2:
    ...
    break;
  case SEDIT_MIN_OBJ_LEVEL:
    S_MIN_OBJ_LEVEL(OLC_SHOP(d)) = MAX(1, MIN(38, atoi(arg)));
    break;
  case SEDIT_MAX_OBJ_LEVEL:
    S_MAX_OBJ_LEVEL(OLC_SHOP(d)) = MAX(1, MIN(38, atoi(arg)));
    break;
