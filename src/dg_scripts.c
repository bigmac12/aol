...
extern const char *pc_pos_types[];
  ...
  if (!*field) {
	...
	*str = '\0';
	...
	sprintf(str, "%d", GET_ALIGNMENT(c));
      ...
      else if (!str_cmp(field, "ethos"))
        sprintf(str, "%d", GET_ETHOS(c));

      else if (!str_cmp(field, "prisoner"))
        sprintf(str, "%d", PLR_FLAGGED(c, PLR_PRISONER));
        ...
        if(IS_NPC(c))
          ...
          sprintf(str, "%d", GET_CLANLEVEL(c));

      else if (!str_cmp(field, "hometown"))
        sprintf(str, "%d", GET_HOME(c));

      else if (!str_cmp(field, "rpfactor"))
        if(IS_NPC(c))
          sprintf(str, "%d", 0);
        else
          sprintf(str, "%d", GET_RPFACTOR(c));

      else if (!str_cmp(field, "gold"))
	sprintf(str, "%d", money_to_copper(GET_MONEY(c)));
...
/*      else if (!str_cmp(field, "race"))
        sprintf(str, "%s", pc_race_types[GET_RACE(ch)]);
*/
      else if (!str_cmp(field, "vnum"))
	sprintf(str, "%d", GET_ACTOR_VNUM(c));\

      else if (!str_cmp(field, "str"))
        sprintf(str, "%d", GET_STR(c));

      else if (!str_cmp(field, "killer"))
        sprintf(str, "%d", is_killer(c));

      else if (!str_cmp(field, "thief"))
        sprintf(str, "%d", is_thief(c));

      else if (!str_cmp(field, "pos"))
        sprinttype(GET_POS(c), pc_pos_types, str);
