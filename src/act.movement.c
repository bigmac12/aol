...
int do_simple_move(struct char_data *ch, int dir, int need_specials_check) {
    ...
    */
    ...
    /* PDH  4/12/99 - char can only enter Open zone  
...
void do_doorcmd(struct char_data *ch, struct obj_data *obj, int door, int scmd) {
    ...
    switch (scmd) {
        ...
        case SCMD_PICK:
            ...
            strlcpy(buf, "$n skillfully picks the lock on ", sizeof(buf));
            break;

        default:
            log("SYSERR: Invalid door command %d passed to do_doorcmd", scmd);
            return;
    }
    ...
    if ((scmd == SCMD_OPEN || scmd == SCMD_CLOSE) && back) {
        ...
        if (world[EXIT(ch, door)->to_room].people) {
            struct char_data *people = world[EXIT(ch, door)->to_room].people;
            act(buf, FALSE, people, 0, 0, TO_ROOM);
            act(buf, FALSE, people, 0, 0, TO_CHAR);
        }
    }
}
...
ACMD(do_enter) {
    ...
    if (*buf) { /* an argument was supplied, search for door or portal keyword */
        for (door = 0; door < NUM_OF_DIRS; door++) {
        ...
        /* Check for portal */
        portal = get_obj_in_list_vis(ch, buf, world[ch->in_room].contents);
        if (portal && GET_OBJ_VNUM(portal) == PORTAL_OBJ) {
            portDestRoom = GET_OBJ_VAL(portal, 0);

            if (portDestRoom < 0 || portDestRoom >= top_of_world) {
                log("SYSERR: portal destination room %d is invalid.", portDestRoom);
                send_to_char("Problem with portal - contact an Immortal.\r\n", ch);
                return;
            }

            snprintf(buf2, sizeof(buf2), "$n enters %s.", portal->short_description);
            act(buf2, TRUE, ch, 0, 0, TO_ROOM);
            ...
            snprintf(buf2, sizeof(buf2), "$n drifts out of %s.", portal->short_description);
            act(buf2, TRUE, ch, 0, 0, TO_ROOM);
            return;
        }
    ...
    for (door = 0; door < NUM_OF_DIRS; door++) {
        if (EXIT(ch, door) && EXIT(ch, door)->to_room != NOWHERE) {
            if (!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED) && IS_SET(ROOM_FLAGS(EXIT(ch, door)->to_room), ROOM_INDOORS)) {
                perform_move(ch, door, 1);
                return;
            }
        }
...
ACMD(do_mount) {
    ...
    if (IS_NPC(vict) && 
        !AFF_FLAGGED(vict, AFF_TAMED) && 
        !AFF_FLAGGED(vict, AFF_CHARM) && 
        GET_SKILL(ch, SKILL_RIDING) <= number(1, 55)) {
        
        act("$N suddenly bucks upwards, throwing you violently to the ground!", 
            FALSE, ch, 0, vict, TO_CHAR);
        act("$n is thrown to the ground as $N violently bucks!", 
            TRUE, ch, 0, vict, TO_NOTVICT);
        act("You buck violently and throw $n to the ground.", 
