...
struct shop_data {
   ...
   int	 bankAccount;		/* Store all copper over 15000(disabled)*/
   int	 lastsort;		/* How many items are sorted in inven?	*/
   int   minObjLevel;           /* Minimum level of object purchased    */
   int   maxObjLevel;           /* Maximum level of object purchased    */
...
#define IS_GOD(ch)		(!IS_NPC(ch) && (GET_LEVEL(ch) >= LVL_DEMIGOD))
...
#define OBJECT_TOO_LOW_LEVEL    4
#define OBJECT_TOO_HIGH_LEVEL   5
...
#define SHOP_MIN_OBJ_LEVEL(i)	(shop_index[(i)].minObjLevel)
#define SHOP_MAX_OBJ_LEVEL(i)	(shop_index[(i)].maxObjLevel)
