...
void mudlog(const char *str, int type, int level, int file) {
  ...
  for (i = descriptor_list; i; i = i->next) {
    // Do not display the mudlog if you are: not CON_PLAYING, switched into an NPC, or lower level than the player who provoked the log
    // if ((STATE(i) != CON_PLAYING || IS_NPC(i->character)) || PLR_FLAGGED(i->character, PLR_WRITING) || GET_LEVEL(i->character) < level) {
    //     continue;
    // }
...
void immcharlog(const char *str) {
    ...
    for (i = descriptor_list; i; i = i->next) {
        // Do not show the immlog if you are: not CON_PLAYING, switched into an NPC, or writing
        // if ((STATE(i) != CON_PLAYING || IS_NPC(i->character)) || PLR_FLAGGED(i->character, PLR_WRITING)) {
        //     continue;
        // }
...
size_t new_sprintbit(bitvector_t bitvector, const char *names[], char *result, size_t reslen) {
    size_t len = 0;
    int nlen;
    long nr;
...
/* Check if making CH follow VICTIM will create an illegal follow "loop/circle" */
...
void stop_follower(struct char_data * ch) {
    ...
    if (IS_AFFECTED(ch, AFF_CHARM)) {
        act("You realize that you can no longer follow $N!", FALSE, ch, 0, chMaster, TO_CHAR);
...
void add_follower(struct char_data * ch, struct char_data * leader) {
    ...
    if (AFF_FLAGGED(ch, AFF_SNEAK) && check_skill(ch, 101, SKILL_SNEAK) && !AFF_FLAGGED(leader, AFF_SENSE_LIFE)) {
        return;
    }
...
void format_text(char **ptr_string, int mode, struct descriptor_data *d, int maxlen) {
    ...
    char formated[MAX_STRING_LENGTH]; /* warning: do not edit messages with max_str's of over this value */
...
char * describe_money(struct money_data amount) {
    ...
    if (amount.platinum > 0) {
        sprintf(describeMoneyBuf, "%s%s%d platinum", describeMoneyBuf, (comma ? ", " : ""), amount.platinum);
        comma++;
    ...
    if (amount.steel > 0) {
        sprintf(describeMoneyBuf, "%s%s%d steel", describeMoneyBuf, (comma ? ", " : ""), amount.steel);
        comma++;
    ...
    if (amount.gold > 0) {
        sprintf(describeMoneyBuf, "%s%s%d gold", describeMoneyBuf, (comma ? ", " : ""), amount.gold);
        comma++;
    ...
    if (amount.copper > 0) {
        sprintf(describeMoneyBuf, "%s%s%d copper", describeMoneyBuf, (comma ? ", " : ""), amount.copper);
        comma++;
    ...
    // This can be optimized. -- Sirr, 2023
    if (!comma) {
...
int money_compare(struct money_data more, struct money_data less, int strict) {
    ...
    if ((temp.platinum < 0) || (temp.gold < 0) || (temp.steel < 0) || (temp.copper < 0)) {
        return -1;
    }
...
void check_improve(struct char_data * ch, int skill) {
    ...
    if ((ch->player_specials->saved.skills[skill].reflections) >= spell_info[skill].max_day) {
        return;
    }
...
int use_skill(struct char_data * ch, int percent, int skillnum) {
    ...
    if (IS_NPC(ch) && ((IS_AFFECTED(ch, AFF_SNEAK) && skillnum == SKILL_SNEAK) || (IS_AFFECTED(ch, AFF_HIDE) && skillnum == SKILL_HIDE))) {
        skillval = 90;
    }
...
int is_color(struct char_data *ch) {
    ...
    // return (PRF_FLAGGED(ch, PRF_COLOR_1) || PRF_FLAGGED(ch, PRF_COLOR_2)) || (IS_NPC(ch) && MOB_FLAGGED(ch, MOB_QUEST)) ? 1 : 0;
}
...
int is_killer(struct char_data *ch) {
    ...
    // return PLR_FLAGGED(ch, PLR_KILLER) ? 1 : 0;
}
...
int is_thief(struct char_data *ch) {
    ...
    // return PLR_FLAGGED(ch, PLR_THIEF) ? 1 : 0;
}
